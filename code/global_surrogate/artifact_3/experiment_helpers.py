...
OUTPUT_ARTIFACT_DIR = Path("artifact_3")
...
def load_mixed_synthetic_dataset(vae_path: Path) -> tuple[pd.DataFrame, list[str], dict]:
    perm_path = SOURCE_ARTIFACT_DIR / "synthetic_local_permutation_300k.csv"
    perm_df = pd.read_csv(perm_path).astype(np.float32)
    vae_df = pd.read_csv(vae_path).astype(np.float32)
    vae_sample = vae_df.sample(n=VAE_SAMPLE_SIZE, random_state=RANDOM_STATE).reset_index(drop=True)
    combined_df = pd.concat([perm_df, vae_sample], axis=0, ignore_index=True)
    feature_cols = combined_df.columns.tolist()
    mix_info = {
        "local_permutation_path": str(perm_path.resolve()),
        "vae_path": str(vae_path.resolve()),
        "local_permutation_rows": int(len(perm_df)),
        "vae_total_rows": int(len(vae_df)),
        "vae_sample_rows": int(len(vae_sample)),
        "combined_rows": int(len(combined_df)),
        "feature_count": int(len(feature_cols)),
    }
    return combined_df, feature_cols, mix_info
...
def load_or_create_teacher_test_cache(
    ...
    legacy_cache_path = Path("artifact_2") / "surrogates" / cache_path.name
    if legacy_cache_path.exists():
        logger.log(f"Reusing teacher prediction cache from artifact_2: {legacy_cache_path}")
