...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train artifact_4 GAMI-Net surrogates on real-train teacher labels.")
...
def main() -> None:
    ...
    logger = Logger(LOG_DIR / f"artifact4_gaminet_{args.variant_name}_{timestamp}.log")
    log = logger.log

    log(f"Starting artifact_4 GAMI-Net surrogate variant: {args.variant_name}")
    log(
    ...
    log(f"Processed splits ready: train_rows={len(train_df)}, test_rows={len(X_test_df)}, features={len(feature_cols)}")

    summary_path = SURROGATE_DIR / f"gaminet_artifact4_{args.variant_name}_summary.json"
    summary = {
        "experiment": f"artifact_4_gaminet_{args.variant_name}",
        "config": {
    ...
    for teacher_name in args.teachers:
        ...
        result = {
            ...
            "artifacts": {"model_pickle": str(model_pickle_path.resolve()), "output_dir": str(output_dir.resolve())},
        }
        summary["teachers"][teacher_name] = result
        save_json(summary, summary_path)
        log(
            f"Finished GAMI-Net for {teacher_name} | holdout_acc={result['holdout_fidelity']['accuracy']:.4f}, "
        ...
        cleanup_memory(
            X_train_df, X_holdout_df, X_train_scaled, X_holdout_scaled, X_test_scaled,
            y_train_gami, y_train, y_holdout, holdout_pred, test_pred, model, teacher_dataset, teacher_labels,
        )

    save_json(summary, summary_path)
    log(f"artifact_4 GAMI-Net surrogate variant completed: {args.variant_name}")
