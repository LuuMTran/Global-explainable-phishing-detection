...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect artifact_2 variant reports into one CSV and JSON.")
...
def main() -> None:
    ...
    ebm_summary = load_json(SURROGATE_DIR / f"ebm_artifact2_{args.variant_name}_summary.json")
    gaminet_summary = None
    if args.include_gaminet:
        gaminet_path = SURROGATE_DIR / f"gaminet_artifact2_{args.variant_name}_summary.json"
        if gaminet_path.exists():
            gaminet_summary = load_json(gaminet_path)

    report = {
        "experiment": f"artifact_2_{args.variant_name}",
        "config": {
    ...
    summary_csv_path = OUTPUT_ARTIFACT_DIR / f"artifact2_{args.variant_name}_experiment_report.csv"
    summary_json_path = OUTPUT_ARTIFACT_DIR / f"artifact2_{args.variant_name}_experiment_report.json"
