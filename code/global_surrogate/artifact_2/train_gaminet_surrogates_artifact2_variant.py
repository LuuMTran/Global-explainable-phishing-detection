...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run artifact_2 GAMI-Net surrogate variants.")
    parser.add_argument(
        "--variant-name",
        required=True,
...
def main() -> None:
    ...
    log_name = f"artifact2_gaminet_{args.variant_name}_{timestamp}.log"
    logger = Logger(LOG_DIR / log_name)
    log = logger.log

    log(f"Starting artifact_2 GAMI-Net surrogate variant: {args.variant_name}")
    ...
    summary_path = SURROGATE_DIR / f"gaminet_artifact2_{args.variant_name}_summary.json"
    summary = {
        "experiment": f"artifact_2_gaminet_{args.variant_name}",
        "config": {
    ...
    for teacher_name in args.teachers:
        ...
        if not teacher_dataset_path.exists():
            raise FileNotFoundError(
                ...
                "Run the matching artifact_2 EBM variant first."
            )
    ...
    save_json(summary, summary_path)
    log(f"artifact_2 GAMI-Net surrogate variant completed: {args.variant_name}")
