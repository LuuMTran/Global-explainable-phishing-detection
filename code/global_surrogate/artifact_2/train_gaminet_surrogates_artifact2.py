...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the artifact_2 GAMI-Net surrogate experiment.")
    parser.add_argument(
        "--teachers",
        nargs="+",
...
def main() -> None:
    ...
    logger = Logger(LOG_DIR / f"artifact2_gaminet_{timestamp}.log")
    log = logger.log

    log("Starting artifact_2 GAMI-Net surrogate experiment")
    ...
    for teacher_name in args.teachers:
        ...
        if not teacher_dataset_path.exists():
            raise FileNotFoundError(
                ...
                "Run the artifact_2 EBM stage first."
            )
        ...
        save_json(summary, SURROGATE_DIR / "gaminet_artifact2_summary.json")
        log(
            f"Finished GAMI-Net for {teacher_name} | train_fidelity_acc={result['synthetic_holdout_fidelity']['accuracy']:.4f}, "
    ...
    save_json(summary, SURROGATE_DIR / "gaminet_artifact2_summary.json")
    log("artifact_2 GAMI-Net surrogate experiment completed")
